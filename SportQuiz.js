...
import { quizData } from './SportQuizData';
import * as SQLite from 'expo-sqlite';
import { LinearGradient } from 'expo-linear-gradient';

const db = SQLite.openDatabase('SportQuizDB.db');
...
export default function SportQuiz() {
    ...
    useEffect(() => {
        db.transaction(tx => {
            tx.executeSql('create table if not exists highScoreSport (id integer primary key autoincrement, name text, score integer);');
        }, null, updateList);
    ...
    const saveScore = () => {
        ...
        db.transaction(tx => {
            tx.executeSql('insert into highScoreSport (name, score) values (?, ?);', [name, score], (_, result) => {
    ...
    const updateList = () => {
        db.transaction(tx => {
            tx.executeSql('select * from highScoreSport order by score desc;', [], (_, { rows }) => {
